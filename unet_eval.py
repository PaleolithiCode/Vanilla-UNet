...
def make_dataset(root = 'carpalTunnel', mask = 'CT', patient_id = None):
    img_list = []
    src_dir = ['T1', 'T2']
...
class CarpalTunnelDataSet(Dataset):
    def __init__(self, root, mask, transform = None, mask_transform = None, patient_id = None):
        dataset = make_dataset(root, mask, patient_id)
...
def eval_model(model, dataload, log_name):
    global device
    ...
    with torch.no_grad():
        ...
        for img, mask in dataload:
            val_step += 1
            print("{}/{}".format(val_step, len(dataload)))
        ...
        with open('./eval_logs_bce/{}'.format(log_name), 'a', encoding = 'utf-8') as fileOut:
            fileOut.write("acc : {:.3f}\n".format(val_acc / len(dataload)))
...
class UnetCt():
    ...
    def eval(self):
        for i in range(10):
            self.dataset = CarpalTunnelDataSet(self.root, self.target, transform = x_transforms, mask_transform = y_transforms, patient_id = i)
            self.data_loader = DataLoader(self.dataset, batch_size = 1)
            eval_model(self.model, self.data_loader, "p{}_{}_b{}_e{}.logs".format(i, self.target, self.batch_size, self.epoch))
...
if __name__ == "__main__":
    ...
    # mn_unet.load('./models/bsize=8/weights_MN_20.pth')
    # ft_unet.load('./models/bsize=8/weights_FT_20.pth')
    # ct_unet.load('./models/bsize=8/weights_CT_20.pth')
    ...
    mn_unet.load('./models/b3_bcewl/weights_b3_MN_20.pth')
    ft_unet.load('./models/b3_bcewl/weights_b3_FT_20.pth')
    ct_unet.load('./models/b3_bcewl/weights_b3_CT_20.pth')
